<?php
namespace Sdtech\CoinpaymentsLaravel\Service;
/*
	CoinPayments.net API Class - v1.1
	Copyright 2022 CoinPayments.net. All rights reserved.
*/

class CoinPaymentLaravelService
{
    private $private_key = '';
    private $public_key = '';
    private $ipn_url = '';
    private $ipn_secret = '';
    private $ipn_merchant_id = '';
    private $ch = null;

    public function __construct()
    {
        $this->public_key = config('coinpayments.COIN_PAYMENT_PUBLIC_KEY');
        $this->private_key = config('coinpayments.COIN_PAYMENT_PRIVATE_KEY');
        $this->ipn_url = config('coinpayments.COIN_PAYMENT_IPN_URL');
        $this->ipn_secret = config('coinpayments.COIN_PAYMENT_IPN_SECRET');
        $this->ipn_merchant_id = config('coinpayments.COIN_PAYMENT_IPN_MERCHANT_ID');
        $this->ch = null;
    }
    ...
    /**
     * Gets the basic account information
     */
    ...
    public function getRates($short = TRUE) {
        $short = $short ? 1:0;
        return $this->api_call('rates', array('short' => $short, 'accepted' => 1));
    }
    ...
    public function createTransaction($amount, $currency1, $currency2, $buyer_email, $address='', $ipn_url='') {
        $req = array(
            'amount' => $amount,
            'currency1' => $currency1,
            'currency2' => $currency2,
    ...
    /**
     * cancel a withdrawal from your account to a specified withdrawal id.<br />
     * @param id The withdrawal ID to cancel. Note the withdrawal must be in the "Awaiting email confirmation" state to be able to be cancelled.
     *
     */
